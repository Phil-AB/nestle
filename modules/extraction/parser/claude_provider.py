    ...
    def _build_extract_user_message(
        ...
        if mode == "open":
            return (
                ...
                "UNLABELED BLOCKS: When an address or party block has NO explicit label (no header text, "
                "just a company name and address), infer its role from context and position and use a "
                "canonical name: shipper_name/shipper_address (sender), consignee_name/consignee_address "
                "(receiver), bill_to_name/bill_to_address (billing party), delivery_name/delivery_address "
                "(delivery destination). Do NOT use the company name or literal text as the field key.\n"
